import { View, Text, TextInput, StyleSheet, ScrollView, TouchableOpacity, KeyboardAvoidingView, Platform, Animated, ActivityIndicator, Alert } from "react-native";
import { router } from "expo-router";
import { useState, useRef, useEffect } from "react";
import { useAudioRecorder, AudioModule, RecordingPresets, setAudioModeAsync } from "expo-audio";
import { enrollDoctorApi } from "../api/auth.api";
...
export default function RegisterScreen() {
  ...
  const [sampleUris, setSampleUris] = useState<(string | null)[]>([null, null, null]);
  ...
  const startStopRecording = async (index: number) => {
    // Stop current recording for this index
    if (recording === index) {
      try {
      ...
      if (uri) {
        setSampleUris((prev) => prev.map((v, i) => (i === index ? uri : v)));
    ...
    // Request permissions and start new recording
    try {
      ...
      Alert.alert("Error", "Could not start recording: " + (err?.message ?? "unknown"));
    }
  };

  const clearSample = async (index: number) => {
    if (recording === index) {
      try {
        await audioRecorder.stop();
      } catch (e) {
    ...
    setSampleUris((prev) => prev.map((v, i) => (i === index ? null : v)));
  };

  const handleRegister = async () => {
    if (sampleCount !== 3) return;
    const uris = sampleUris as [string, string, string];
    if (uris.some((u) => !u)) {
      Alert.alert("Missing Audio", "Please record all 3 voice samples before submitting.");
      return;
    }
    setSubmitting(true);
    setSubmitError(null);
    try {
      await enrollDoctorApi(name.trim(), password, uris);
      Alert.alert("Enrolled!", "Your account has been created. Please sign in.", [
        { text: "OK", onPress: () => router.replace("/(auth)/login") },
      ]);
    } catch (err: any) {
      let msg = err?.response?.data?.detail ?? err?.message ?? "Enrollment failed.";
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <Animated.View style={[styles.mainGrid, { opacity: fadeAnim }]}>
          ...
          <View style={styles.submitRow}>
            ...
            <TouchableOpacity
              style={[styles.submitBtn, (sampleCount !== 3 || submitting) && styles.submitBtnDisabled]}
              onPress={handleRegister}
              disabled={sampleCount !== 3 || submitting}
            >
              {submitting
                ? <ActivityIndicator color="#f2f7f1" />
